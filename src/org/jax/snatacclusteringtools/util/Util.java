package org.jax.snatacclusteringtools.util;
...
public class Util {
	...
	public String getBarcode(SAMRecord read) {
	...
	public TreeMap<String, Integer> readChromSizes(String chromsizefile) throws IOException{
	...
	public TreeMap<String, String> readCellBarcodes(String chromsizefile) throws IOException{
		BufferedReader br = new BufferedReader(new FileReader(chromsizefile));
		TreeMap<String, String> rv = new TreeMap<String, String>();
		while(br.ready()) {
			String line = br.readLine();
			String[] split = line.split(",");
			boolean iscellbarcode = split[9].equals("1");
			if(iscellbarcode) {
				String barcode = split[0];
				String cellid = split[8];
